...
import { GetNutritionInfo } from "./subcomponts/algorithms";
import DetectClickOutsideComponent from "./subcomponts/clickedOutSideRef";


function LogForm (props) {
  const {setLogFoodEntryIsOpen, foodLogEntry, setFoodLogEntry, selectedMeal, setNutritionInfo} = props;
  ...
  function handleFormSubmit (e) {
    ...
    const newFormEntry = {...logEntry};
    newFormEntry["type"] = "item"
    
    //if nothings is enterd for item name is Quick add will be default
    if (!newFormEntry.name) newFormEntry.name = "Quick Add"


    //add the new log to the data
    setFoodLogEntry((prev)=>{
      const newLog = prev
      newLog[selectedMeal].push(newFormEntry)
      return newLog
    })

    //update nutrition info
    setNutritionInfo(()=>GetNutritionInfo(foodLogEntry))

    //close the form
    setLogFoodEntryIsOpen(false)    
  }
    ...
    <form ref={logWrapperRef} onSubmit={(e)=>{handleFormSubmit(e)}} className="absolute flex flex-wrap justify-center w-5/6 p-2 -translate-x-1/2 -translate-y-1/2 shadow-md shadow-black/45 top-1/2 left-1/2 log-form">
      ...
      <div className="flex flex-wrap justify-around mt-3">
        ...
        <div className="flex flex-wrap justify-center" id="servings-fill">
          <div className="w-full p-3">
          <svg width="24" height="24" className="m-auto" fillRule="evenodd" clipRule="evenodd">
